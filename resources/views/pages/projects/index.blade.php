@extends('layouts.main')
@section('content')
<div
              class="vc_row wpb_row vc_row-fluid vc_custom_1602471563695 fl-helping-row-responsive-64dc649bef746-5295">
              <div class="wpb_column vc_column_container vc_col-sm-12">
                <div class=" vc_column-inner  ">
                  <div class="wpb_wrapper">
                    <div class="fl-vc-custom-title-container text-center custom-title">
                      <div class="custom-title-content-wrapper">
                        <!--<div class="fl-subtitle-vc fl-font-style-semi-bolt vc-custom-sub-title">DESIGN AND-->
                        <!--  BUILD</div>-->
                        <h2
                          class="fl-title-vc vc-custom-title fl-font-style-bolt fl-title-responsive-64dc649bef9b5-656 fl-vc--responsive">
                           <span class="fl-primary-color">Our Projects</span></h2>
                      </div>
                    </div>
                    <div class="fl-vc-responsive--gap cf fl-responsive-gap-64dc649bef9fa-4533 fl-vc--responsive"
                      data-responsive-target='fl-responsive-gap-64dc649bef9fa-4533'
                      data-responsive-param='{"height":"mobile:15px;"}' style=height:25px;></div>
                    <div class="fl-resent-cars-vc">

                      <div class="resent-cars-container fl-animated-item-velocity" data-animate-type="slideRightIn"
                        data-item-for-animated=".resent-car-item">
                
                           <!--article start-->
                         <article class="resent-car-item">
                          <!--article start-->

                          <div class="entry-auto-grid-wrap">
                            <!--.entry-auto-grid-wrap -->

                            <div class="top-content-wrap">

                            
                              <div class="image-grid-mask"></div>
                              <a class="auto-link-image-link" href="{{ url('mbweni')}}"></a>

                              <img src="{{ asset('assets/images/uploads/2019/07/mbweni.jpg')}}"
                                class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                            </div>
                            <div class="bottom-content-wrap">
                              <h6 class="title-content">
                                <span class="grid-auto-title fl-text-title-style">Proposed Construction of
                                  Residential House</span>
                              </h6>
                              <div class="auto-grid-list-content">
                                <ul class="list-grid--info">
                                  <li class="list-grid-item fl-font-style-regular"> Mbweni, Dar es Salaam</li>

                                </ul>
                              </div>
                            </div>
                          </div>
                          <!--//.entry-auto-grid-wrap -->
                        </article>
                        <!--article end-->
                         <!--article start-->
                         <article class="resent-car-item">
                          <!--article start-->

                          <div class="entry-auto-grid-wrap">
                            <!--.entry-auto-grid-wrap -->

                            <div class="top-content-wrap">

                            
                              <div class="image-grid-mask"></div>
                              <a class="auto-link-image-link" href="{{ url('night-club')}}" ></a>

                              <img src="{{ asset('assets/images/uploads/2019/07/night-club-masaki.jpg')}}"
                                class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                            </div>
                            <div class="bottom-content-wrap">
                              <h6 class="title-content">
                                <span class="grid-auto-title fl-text-title-style">Proposed Construction of
                                  Night Club</span>
                              </h6>
                              <div class="auto-grid-list-content">
                                <ul class="list-grid--info">
                                  <li class="list-grid-item fl-font-style-regular"> <br><br> Masaki, Dar es Salaam</li>

                                </ul>
                              </div>
                            </div>
                          </div>
                          <!--//.entry-auto-grid-wrap -->
                        </article>
                        <!--article end-->
                        
                          <!--article start-->
                          <article class="resent-car-item">
                            <!--article start-->
  
                            <div class="entry-auto-grid-wrap">
                              <!--.entry-auto-grid-wrap -->
  
                              <div class="top-content-wrap">
  
                              
                                <div class="image-grid-mask"></div>
                                <a class="auto-link-image-link" href="{{ url('T-Mark')}}"></a>
  
                                <img src="{{ asset('assets/images/uploads/2019/07/bahari-beach.png')}}"
                                  class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                              </div>
                              <div class="bottom-content-wrap">
                                <h6 class="title-content">
                                  <span class="grid-auto-title fl-text-title-style">Proposed Renovation of
                                    T-mark Offices </span>
                                </h6>
                                <div class="auto-grid-list-content">
                                  <ul class="list-grid--info">
                                    <li class="list-grid-item fl-font-style-regular">Bahari Beach, Dar es Salaam</li>
  
                                  </ul>
                                </div>
                              </div>
                            </div>
                            <!--//.entry-auto-grid-wrap -->
                          </article>
                          <!--article end-->
                           <!--article start-->
                         <article class="resent-car-item">
                          <!--article start-->

                          <div class="entry-auto-grid-wrap">
                            <!--.entry-auto-grid-wrap -->

                            <div class="top-content-wrap">

                            
                              <div class="image-grid-mask"></div>
                              <a class="auto-link-image-link" href="{{ url('DRC-Embassy')}}"></a>

                              <img src="{{ asset('assets/images/uploads/2019/07/mbweni.jpg')}}"
                                class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                            </div>
                            <div class="bottom-content-wrap">
                              <h6 class="title-content">
                                <span class="grid-auto-title fl-text-title-style">Proposed Rennovation of DRC Embassy</span>
                              </h6>
                              <div class="auto-grid-list-content">
                                <ul class="list-grid--info">
                                  <li class="list-grid-item fl-font-style-regular">Dar es Salaam</li>

                                </ul>
                              </div>
                            </div>
                          </div>
                          <!--//.entry-auto-grid-wrap -->
                        </article>
                        <!--article end-->
                         <!--article start-->
                         <article class="resent-car-item">
                          <!--article start-->

                          <div class="entry-auto-grid-wrap">
                            <!--.entry-auto-grid-wrap -->

                            <div class="top-content-wrap">

                            
                              <div class="image-grid-mask"></div>
                              <a class="auto-link-image-link" href="{{ url('PSI-Tanzania')}}"></a>

                              <img src="{{ asset('assets/Photos/psi/psi-2.png')}}"
                                class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                            </div>
                            <div class="bottom-content-wrap">
                              <h6 class="title-content">
                                <span class="grid-auto-title fl-text-title-style">Proposed Design and Partition of Warehouse for PSI-Tanzania </span>
                              </h6>
                              <div class="auto-grid-list-content">
                                <ul class="list-grid--info">
                                  <li class="list-grid-item fl-font-style-regular">Mikocheni, Dar es Salaam</li>

                                </ul>
                              </div>
                            </div>
                          </div>
                          <!--//.entry-auto-grid-wrap -->
                        </article>
                        <!--article end-->
                          <!--article start-->
                          <article class="resent-car-item">
                            <!--article start-->
  
                            <div class="entry-auto-grid-wrap">
                              <!--.entry-auto-grid-wrap -->
  
                              <div class="top-content-wrap">
  
                              
                                <div class="image-grid-mask"></div>
                                <a class="auto-link-image-link" href="{{ url('Kisota')}}"></a>
  
                                <img src="{{ asset('assets/Photos/kisota/kisota-1.png')}}"
                                  class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                              </div>
                              <div class="bottom-content-wrap">
                                <h6 class="title-content">
                                  <span class="grid-auto-title fl-text-title-style">Proposed Construction of Residential House in Kisota</span>
                                </h6>
                                <div class="auto-grid-list-content">
                                  <ul class="list-grid--info">
                                    <li class="list-grid-item fl-font-style-regular">Kigamboni, Dar es Salaam</li>
  
                                  </ul>
                                </div>
                              </div>
                            </div>
                            <!--//.entry-auto-grid-wrap -->
                          </article>
                          <!--article end-->
                           <!--article start-->
                         <article class="resent-car-item">
                          <!--article start-->

                          <div class="entry-auto-grid-wrap">
                            <!--.entry-auto-grid-wrap -->

                            <div class="top-content-wrap">

                            
                              <div class="image-grid-mask"></div>
                              <a class="auto-link-image-link" href="{{ url('ZIC')}}"></a>

                              <img src="{{ asset('assets/Photos/ZIC/48.jpg')}}"
                                class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                            </div>
                            <div class="bottom-content-wrap">
                              <h6 class="title-content">
                                <span class="grid-auto-title fl-text-title-style">Proposed Rehabilitation of Zanzibar Insurance Corporation </span>
                              </h6>
                              <div class="auto-grid-list-content">
                                <ul class="list-grid--info">
                                  <li class="list-grid-item fl-font-style-regular"><br><br> Lumumba, Dar es Salaam</li>

                                </ul>
                              </div>
                            </div>
                          </div>
                          <!--//.entry-auto-grid-wrap -->
                        </article>
                        <!--article end-->
                         <!--article start-->
                         <article class="resent-car-item">
                          <!--article start-->

                          <div class="entry-auto-grid-wrap">
                            <!--.entry-auto-grid-wrap -->

                            <div class="top-content-wrap">

                            
                              <div class="image-grid-mask"></div>
                              <a class="auto-link-image-link" href="{{ url('TPDC')}}"></a>

                              <img src="{{ asset('assets/Photos/TPDC/3.jpg')}}"
                                class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                            </div>
                            <div class="bottom-content-wrap">
                              <h6 class="title-content">
                                <span class="grid-auto-title fl-text-title-style">Proposed Renovation of Gas Station for TPDC</span>
                              </h6>
                              <div class="auto-grid-list-content">
                                <ul class="list-grid--info">
                                  <li class="list-grid-item fl-font-style-regular"><br><br> Tanita-Kibaha, Dar es Salaam</li>

                                </ul>
                              </div>
                            </div>
                          </div>
                          <!--//.entry-auto-grid-wrap -->
                        </article>
                        <!--article end-->
                          <!--article start-->
                          <article class="resent-car-item">
                            <!--article start-->
  
                            <div class="entry-auto-grid-wrap">
                              <!--.entry-auto-grid-wrap -->
  
                              <div class="top-content-wrap">
  
                              
                                <div class="image-grid-mask"></div>
                                <a class="auto-link-image-link" href="{{ url('CFAO')}}"></a>
  
                                <img src="{{ asset('assets/Photos/CFAO/cfao-main.png')}}"
                                  class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                              </div>
                              <div class="bottom-content-wrap">
                                <h6 class="title-content">
                                  <span class="grid-auto-title fl-text-title-style">Proposed roof repairs and renovation for CFAO Motors Tanzania</span>
                                </h6>
                                <div class="auto-grid-list-content">
                                  <ul class="list-grid--info">
                                    <li class="list-grid-item fl-font-style-regular"><br><br> Dar es Salaam</li>
  
                                  </ul>
                                </div>
                              </div>
                            </div>
                            <!--//.entry-auto-grid-wrap -->
                          </article>
                          <!--article end-->
                           <!--article start-->
                           <article class="resent-car-item">
                            <!--article start-->
  
                            <div class="entry-auto-grid-wrap">
                              <!--.entry-auto-grid-wrap -->
  
                              <div class="top-content-wrap">
  
                              
                                <div class="image-grid-mask"></div>
                                <a class="auto-link-image-link" href="{{ url('BLUE-COAST')}}"></a>
  
                                <img src="{{ asset('assets/Photos/BlueCoast-Offices/bluecoast-image-1.jpeg')}}"
                                  class="img-responsive wp-post-image" alt="" decoding="async" loading="lazy" />
                              </div>
                              <div class="bottom-content-wrap">
                                <h6 class="title-content">
                                  <span class="grid-auto-title fl-text-title-style">Proposed Office design and partitions for Blue Cost Investment Ltd</span>
                                </h6>
                                <div class="auto-grid-list-content">
                                  <ul class="list-grid--info">
                                    <li class="list-grid-item fl-font-style-regular">Masaki, Dar es Salaam</li>
  
                                  </ul>
                                </div>
                              </div>
                            </div>
                            <!--//.entry-auto-grid-wrap -->
                          </article>
                          <!--article end-->
                       
                      </div>

                      <div class="cf"></div>
                     
                    </div>
                  </div>
                </div>
              </div>
            </div>

<!-- Content End-->
<!--Padding Bottom Start-->
<div class="fl-page-padding bottom"></div>
<!--Padding Bottom End-->
@endsection